...
const validateInput: (currentAccoutValues: SettingsProps['currentAccoutValues']) => boolean = (
  ...
  if ('email' in currentAccoutValues && !validateEmail(currentAccoutValues['email'] as string)) {
    toast.error(`Invalid input : ensure email is 8 characters long`, notificationStyle);
    isValid = false;
  }

  if (
    'new_password' in currentAccoutValues &&
    'repeat_password' in currentAccoutValues &&
...
export { checkData, validateInput, getInitialData };
