...
function RemoteTournamentHTML({ obj, currentTab, setTournamentID }: RemoteTournamentHTMLTypes): React.JSX.Element {
  return (
    <div key={obj.tournament_name}>
      <TournamentCard
        key={obj.tournament_name}
        name={obj.tournament_name}
        date={currentTab === 'Completed' ? formatDate(obj.tournament_end) : formatDate(obj.tournament_start)}
        ...
        isDisabled={obj.tournament_end !== null ? true : false}
      />
    </div>
  );
}
...
async function getTournamentsTabData(
  ...
  if (data) {
    ...
    Object.values(data[currentTab]).map((obj) => {
      tournamentCards.push(RemoteTournamentHTML({ obj, currentTab, setTournamentID }));
    });
  }
...
function findNeedleInHaystack({
  ...
  currentTab,
  setTournamentID,
}: findNeedleInHaystackTypes): React.JSX.Element[] {
  const tournamentCards: React.JSX.Element[] = [];
      ...
      <section className={`Tournament_section p-0 m-0 ${!isSearching ? 'visible' : 'hide'}`}>
        {choosenTab === 'My Tournament' ? (
          ...
          <>
            <div
              className={
                tournamentsToRender.length > 0 ? `col-12 col-xl-7 col-xxl-7 order-1 p-0 m-0` : `col-12 order-1 p-0 m-0`
              }
            >
