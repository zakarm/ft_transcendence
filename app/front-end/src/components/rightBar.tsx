...
export default function RightBar({
  ...
  const filteredFriends = friends_data
    ...
    .map((user: Friend, index: number) => (
      <Player key={index} nickname={user.username} id={user.id} image={user.image_url} isConnected={user.connected} />
    ));
  ...
  return (
    <Offcanvas
      className={`${styles.offcanvas} border-0`}
      show={show}
      onHide={handleClose}
      placement="end"
      scroll={true}
                          ...
                          >
                            {notifications_data &&
                              notifications_data.map((key: Notification, index: number) => (
                                <Dropdown.Item style={{ background: '#161625' }} key={index} eventKey={index}>
