        ...
        if (data) {
          if (api === 'friends-unblock') {
            ...
            if (unblockedUser) {
              ...
              setBlockedUsers(searchedBlockedFriends.filter((user) => user.user.username !== user_data.user.username));
            }
          }
          if (api === 'friends-remove' || api === 'friends-add' || api === 'friends-block') {
            if (api === 'friends-remove' || api === 'friends-block') {
              ...
              const updatedUsers = searchedFriends.map((item) => {
                if (item.user.username === user_data.user.username) return { ...item, is_user_from: true };
                return item;
              });
  ...
  return (
    <>
      <Modal contentClassName={`${styles.friend_modal}`} show={show} aria-labelledby="add_friend" centered>
                    ...
                    {friend.is_accepted ? (
                      <div className="col-9 d-flex">
                        <div className="col text-end p-1">
                          <Button
                            variant="dark"
                            onClick={() => fetchUser('friends-remove', 'removed from friends', friend)}
                          >
                            Remove <IoIosRemoveCircle color="#FFEBEB" />
                          </Button>
                        </div>
                        <div className="col text-end p-1">
                          <Button variant="dark" onClick={() => fetchUser('friends-block', 'user is blocked', friend)}>
                      ...
                      <div className="col-9 text-end">
                        <Button variant="dark" onClick={() => fetchUser('friends-add', 'Friend Request Sent', friend)}>
              ...
              {blockedUsers &&
                blockedUsers.map((friend, index) => (
                  ...
                  >
                    ...
                    <div className="col-9 text-end">
                      <Button variant="dark" onClick={() => fetchUser('friends-unblock', 'unblocked', friend)}>
        ...
        <Modal.Footer>
          <div className={`${styles.edit_btn} col-md-3 col-sm-5 valo-font text-center m-2 px-2`} onClick={close}>
