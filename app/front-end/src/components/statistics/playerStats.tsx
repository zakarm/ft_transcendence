    ...
    datasets: [
      {
        ...
        backgroundColor: ['rgb(39, 178, 153)', 'rgb(22, 22, 37)'],
        borderColor: '#161625',
        hoverOffset: 4,
      },
    ],
...
function RectangleText({ text, backgroundColor }: { text: string; backgroundColor: string }) {
  return (
    <>
      <h3
        className={`col-12 col-xxl-5 d-flex align-item-center  justify-content-center ${styles.rectangle_text}`}
        style={{ backgroundColor: `${backgroundColor}` }}
      >
        <div className="align-self-center itim-font font-color">{text}</div>
      </h3>
    </>
  );
}
    ...
    <>
      <div className={`col-12 col-xxl-4 d-flex justify-content-center m-1 p-1  ${styles.doughnut_chart_container}`}>
        <div className={`row ${styles.win_rate_text} itim-font font-color`}>
          ...
          <h4 className="text-center">{stats && stats.win_rate ? stats.win_rate.toFixed(2) : '0'}%</h4>
        </div>
