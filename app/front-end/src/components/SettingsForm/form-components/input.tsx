...
function GetListInput({ className = '', labelText = '', id = '', opt = [], choosenPosition = '' }: ListInputProps) {
    ...
    <>
      <div className={`${className} flex-wrap flex-xl-nowrap mb-4 `}>
        ...
        <div className={`${styles.inputHolder} row p-0 m-1`}>
          ...
          >
            {opt.map((val) => (
              <option key={val} className={`itim-font`} value={val}>
                {val}
              </option>
            ))}
          </select>
...
function GetCheckboxInput({ className = 'col', inputType = 'text', inputId = '', labelText = '', inputLength }: Props) {
  ...
  return (
    <div className={`${className} flex-wrap flex-xxl-nowrap`}>
      {twoFaData && <TwoFa value={twoFaData.value} email={twoFaData.email} qr={true} setPassOTP={setPassOTP} />}
      <label
        className={`col-8 col-sm-3 itim-font d-flex align-items-center p-0 m-0 ${styles.inputTitle} ${styles.labelClass}`}
        htmlFor={inputId}
      >
